...
import com.mac.ecomdeliveryphp.databinding.FragmentPickupBinding
...
class PickupFragment : Fragment() {
    ...
    private lateinit var _binding:FragmentPickupBinding
            ...
            { response ->
                try {
                    ...
                    if(success.equals("1")){
                        ...
                        binding.pickupRecycler.setHasFixedSize(true)
                        binding.pickupRecycler.layoutManager=LinearLayoutManager(context)
                        val adapter = context?.let { CommonOrderAdapter(it,list) }
                        if (adapter != null) {
                            adapter.notifyDataSetChanged()
                        }

                        binding.pickupRecycler.adapter=adapter
