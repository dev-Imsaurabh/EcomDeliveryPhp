...
class ActiveFragment : Fragment() {
    ...
    private lateinit var _binding:FragmentActiveBinding
    ...
    override fun onCreateView(
        ...
        _binding = FragmentActiveBinding.inflate(layoutInflater)
            ...
            { response ->
                try {
                    ...
                    if(success.equals("1")){
                        ...
                        binding.activeRecycler.setHasFixedSize(true)
                        binding.activeRecycler.layoutManager=LinearLayoutManager(context)
                        val adapter = context?.let { CommonOrderAdapter(it,list) }
                        if (adapter != null) {
                            adapter.notifyDataSetChanged()
                        }

                        binding.activeRecycler.adapter=adapter
